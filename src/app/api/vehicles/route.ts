...
async function createDefaultVehicles() {
  return Promise.all(defaultVehicles.map((vehicle) =>
    db.vehicle.create({ data: vehicle })
  ));
}
...
export async function GET() {
  try {
    ...
    if (existing === 0) {
      const vehicles = await createDefaultVehicles();
...
export async function POST() {
  try {
    const existing = await db.vehicle.count();
    if (existing > 0) {
      const vehicles = await db.vehicle.findMany({ orderBy: { pricePerHour: 'asc' } });
      return NextResponse.json(vehicles);
    }
